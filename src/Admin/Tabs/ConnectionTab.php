<?php
/**
 * Tab "Connessione": credenziali API e test connessione.
 *
 ...
 * e delle impostazioni GitHub per l'updater.
 */
class ConnectionTab {
	...
	public function render(): void {
		...
		<form method="post" action="">
			...
			<h2><?php esc_html_e( 'Aggiornamenti GitHub', 'wp-alpinebits-reservation' ); ?></h2>
			<p class="description">
				<?php esc_html_e( 'Il plugin verifica automaticamente le release pubblicate su GitHub.', 'wp-alpinebits-reservation' ); ?>
			</p>

			<table class="form-table" role="presentation">
				<tr>
					<th scope="row">
						<label for="wpar_github_owner"><?php esc_html_e( 'GitHub Owner', 'wp-alpinebits-reservation' ); ?></label>
					</th>
					<td>
						<input
							type="text"
							id="wpar_github_owner"
							name="wpar_github_owner"
							class="regular-text"
							value="<?php echo esc_attr( defined( 'WPAR_GITHUB_OWNER' ) ? \WPAR_GITHUB_OWNER : $settings['github_owner'] ); ?>"
							<?php echo defined( 'WPAR_GITHUB_OWNER' ) ? 'disabled' : ''; ?>
						/>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="wpar_github_repo"><?php esc_html_e( 'GitHub Repository', 'wp-alpinebits-reservation' ); ?></label>
					</th>
					<td>
						<input
							type="text"
							id="wpar_github_repo"
							name="wpar_github_repo"
							class="regular-text"
							value="<?php echo esc_attr( defined( 'WPAR_GITHUB_REPO' ) ? \WPAR_GITHUB_REPO : $settings['github_repo'] ); ?>"
							<?php echo defined( 'WPAR_GITHUB_REPO' ) ? 'disabled' : ''; ?>
						/>
					</td>
				</tr>
			</table>
	...
	private function save(): void {
		...
		Options::save_settings(
			array(
				...
				'github_owner'   => isset( $_POST['wpar_github_owner'] ) ? wp_unslash( $_POST['wpar_github_owner'] ) : '',
				'github_repo'    => isset( $_POST['wpar_github_repo'] ) ? wp_unslash( $_POST['wpar_github_repo'] ) : '',
			)
		);
